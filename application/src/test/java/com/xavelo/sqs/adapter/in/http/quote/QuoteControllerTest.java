...
class QuoteControllerTest {
    ...
    void getQuotes() throws Exception {
        List<Quote> quotes = List.of(new Quote("id-1", "q", "s", "a", 1999, "art", 0, 0, null));
    ...
    void getRandomQuoteFound() throws Exception {
        Quote quote = new Quote("id-1", "q", "s", "a", 1999, "art", 0, 0, null);
    ...
    void getQuoteFound() throws Exception {
        ...
        QuoteDto dto = new QuoteDto()
                ...
                .hits(0)
                .spotifyArtistId(null);
        when(getQuoteUseCase.getQuote("id-1")).thenReturn(quote);
        when(quoteMapper.toDto(quote)).thenReturn(dto);

        mockMvc.perform(get("/api/quote/id-1"))
                .andExpect(status().isOk())
                .andExpect(content().json(objectMapper.writeValueAsString(dto)));
    }
    ...
    void getQuoteNotFound() throws Exception {
        when(getQuoteUseCase.getQuote("id-2")).thenReturn(null);

        mockMvc.perform(get("/api/quote/id-2"))
    ...
    void getTop10Quotes() throws Exception {
        List<Quote> quotes = List.of(
                new Quote("id-1", "q1", "s1", "a1", 2000, "art1", 100, 10, null),
                new Quote("id-2", "q2", "s2", "a2", 2001, "art2", 90, 9, null)
        );
