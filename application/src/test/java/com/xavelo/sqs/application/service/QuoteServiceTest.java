...
class QuoteServiceTest {
    ...
    @BeforeEach
    void setUp() {
        sampleQuote = new Quote("id-1", "q", "s", "a", 2000, "artist", 5, 7, null);
    }
    ...
    void storeQuotes_sanitizesAllQuotesAndDelegates() {
        ...
        Quote q2 = new Quote("id-2", "q2", "s2", "a2", 1999, "artist2", 3, 4, null);
    ...
    void getQuote_incrementsHitsAndReturnsUpdatedQuote() {
        when(loadQuotePort.loadQuote("id-1")).thenReturn(sampleQuote);

        Quote result = quoteService.getQuote("id-1");

        verify(incrementHitsPort).incrementHits("id-1");
        verify(quoteEventOutboxPort).recordQuoteHitEvent(result);
        verify(applicationEventPublisher).publishEvent(eventCaptor.capture());
        Object event = eventCaptor.getValue();
        assertTrue(event instanceof QuoteHitEvent);
        assertEquals("id-1", ((QuoteHitEvent) event).quote().id());
